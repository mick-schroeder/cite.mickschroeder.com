...
(async () => {
  ...
  const coreStylesPath = join(
    __dirname,
    "..",
    "data",
    "citation-styles-data.json",
  );
  const renamedStylesPath = join(
    __dirname,
    "..",
    "data",
    "renamed-styles.json",
  ...
  const stylesMeta = JSON.parse(
    await fetchIfNeeded(stylesURL, stylesJsonPath, stylesCacheTime),
  );
  const renamedStyles = JSON.parse(
    await fetchIfNeeded(renamedStylesURL, renamedStylesPath, stylesCacheTime),
  );

  const coreCitationStyles = styles
    .map((style) => {
      if (style in renamedStyles) {
        console.warn(
          `Core citation style "${style}" has been renamed to "${renamedStyles[style]}". Using the new name.`,
        );
