...
const keysToTriggerKeyboardMode = [
  "Tab",
  "ArrowLeft",
  "ArrowRight",
  "ArrowDown",
...
const isInitiallyMouse =
  typeof matchMedia === "function"
    ? matchMedia("(pointer:fine)").matches
    : null;
const isInitiallyTouch =
  typeof matchMedia === "function"
...
const useUserTypeDetector = () => {
  ...
  const [userType, setUserType] = useState({
    mouse: isInitiallyMouse,
    touch: isInitiallyTouch,
    keyboard: false,
  });
  ...
  const handleKeyboard = useCallback((ev) => {
    if (keysToTriggerKeyboardMode.includes(ev.key)) {
      ...
      setUserType((state) =>
        state.keyboard ? state : { ...state, keyboard: true },
      );
    }
  }, []);
  ...
  const handleMouse = useCallback((ev) => {
    ...
    if (
      !lastTouchStartEvent.current ||
      ev.timeStamp - lastTouchStartEvent.current > 500
    ) {
      setUserType((state) =>
        state.mouse && !state.touch
          ? state
          : { ...state, mouse: true, touch: false },
      );
    }
  }, []);
