...
const MultipleLitemsList = forwardRef(
  ({ multipleItems, selectedItems, onItemSelectionChange }, ref) => {
    ...
    const bibliographyRendered = useMemo(() => {
      ...
      const { bibliographyItems, bibliographyMeta, styleHasBibliography } =
        multipleItems;
          ...
          >
            <div
              ...
              dangerouslySetInnerHTML={{
                __html:
                  bibliographyRenderedNodes[index]?.innerHTML || item.value,
              }}
            />
...
MultipleLitemsList.propTypes = {
  multipleItems: PropTypes.shape({
    bibliographyItems: PropTypes.arrayOf(
      PropTypes.shape({
...
const MultipleItemsDialog = (props) => {
  ...
  const { focusNext, focusPrev, receiveBlur, receiveFocus, resetLastFocused } =
    useFocusManager(footerRef, {
      initialQuerySelector: ".btn-outline-secondary:not(:disabled)",
    });
  ...
  useEffect(() => {
    if (
      prevActiveDialog !== activeDialog &&
      activeDialog === "MULTIPLE_ITEMS_DIALOG"
    ) {
      setSelectedItems([]);
    }
  ...
  const title = intl.formatMessage({
    id: "zbib.multipleItems.prompt",
    defaultMessage: "Please select citations from the list",
  });
