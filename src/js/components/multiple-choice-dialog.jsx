  ...
  return (
    ...
    >
      ...
      <h5 id={id} className="title">
        ...
        {badge && (
          <span className="badge badge-light d-xs-none d-sm-inline-block">
            {badge}
          </span>
        )}
      </h5>
...
const getItem = (ev, items) =>
  items.find(
    (item) =>
      item.signature ===
      ev.currentTarget.closest("[data-signature]")?.dataset.signature,
  );
...
const MultipleChoiceDialog = (props) => {
  ...
  const intl = useIntl();
  const useDescriptionColumn = !!multipleChoiceItems?.some(
  ...
  const title = intl.formatMessage({
    id: "zbib.multipleChoice.prompt",
    defaultMessage: "Please select a citation from the list",
  });
        ...
        <div className="modal-body" tabIndex={-1}>
          <ul
            ...
            className={cx("results", {
              "single-column": !useDescriptionColumn,
            })}
          >
