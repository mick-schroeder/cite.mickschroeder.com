...
const StyleSelector = ({ className, citationStyle, citationStyles, onCitationStyleChanged }) => {
	...
	return (
		<div className={ cx('style-selector flex w-full flex-col gap-2 text-left', className ) }>
			<label
				id={`${selectId}-label`}
				htmlFor={ selectId }
			...
			<div className="relative">
				...
				<ChevronDown className="pointer-events-none absolute right-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground transition-colors peer-focus-visible:text-ring" aria-hidden="true" />
			</div>
		</div>
	);
};
