...
const ConfirmAddDialog = (props) => {
  ...
  const currentStyleHtml = useCallback(() => {
    ...
    const { bibliographyItems, bibliographyMeta } =
      itemToConfirm.inCurrentStyle;
    const html = selectedStyle.styleHasBibliography
  ...
  const incomingStyleHtml = useCallback(() => {
    ...
    const html = incomingStyle.styleHasBibliography
      ? formatBib(bibliographyItems, bibliographyMeta)
      : formatFallback(bibliographyItems);
    return html;
  ...
  const title = intl.formatMessage({
    id: "zbib.dialog.confirmAddThisCitation",
    defaultMessage: "Add this citation to your bibliography?",
  });
    ...
    >
      <div className="modal-content" tabIndex={-1}>
        ...
        <div
          ...
          style={
            minModalBodyHeight.current
              ? { minHeight: minModalBodyHeight.current }
              : {}
          }
        ...
        >
          {itemToConfirm.inIncomingStyle ? (
            <>
              ...
              >
                <div
                  dangerouslySetInnerHTML={{ __html: incomingStyleHtml() }}
                />
              </TabPane>
            </>
          ) : (
            ...
            {activeTab === "incoming-style-content" ? (
              (incomingStyle.titleShort ?? incomingStyle.title).length > 40 ? (
                ...
                <FormattedMessage
                  ...
                  values={{
                    style: incomingStyle.titleShort ?? incomingStyle.title,
                  }}
                />
              )
            ) : (
