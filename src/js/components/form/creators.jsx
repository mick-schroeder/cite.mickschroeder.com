...
const Creators = (props) => {
  ...
  const hasVirtual = useMemo(
    () => !!creators.find((creator) => creator[Symbol.for("isVirtual")]),
    [creators],
  );
  ...
  const handleSaveCreators = useCallback(
    (creators) => {
      const newValue = creators
        .filter(
          (creator) =>
            !creator[Symbol.for("isVirtual")] &&
  ...
  const handleValueChanged = useCallback(
    (index, key, value) => {
      ...
      if (
        (newCreators[index].lastName ||
          newCreators[index].firstName ||
          newCreators[index].name) &&
  ...
  const handleReorder = useCallback(
    (fromIndex, toIndex, commit = false) => {
      const newCreators = [...creators];
  ...
  useEffect(() => {
    if (
      typeof prevCreatorTypes !== "undefined" &&
      !deepEqual(creatorTypes, prevCreatorTypes)
    ) {
      ...
      setCreators(
        enumerateObjects(
          creators.map((creator) => ({
            ...creator,
            creatorType: validCreatorTypes.includes(creator.creatorType)
              ? creator.creatorType
              : validCreatorTypes[0],
          })),
        ),
  ...
  useEffect(() => {
    if (creators && prevCreators && creators.length > prevCreators.length) {
      const virtualEntryIndex = creators.findIndex(
        (c) => c[Symbol.for("isVirtual")],
      );
