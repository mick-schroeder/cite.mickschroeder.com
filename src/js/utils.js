...
const ensureNoBlankItems = (itemsCSL) =>
  itemsCSL.map((item) => {
...
const parseIdentifier = (identifier) => {
  ...
  const matches = decodeURIComponent(identifier).match(
    /^https?:\/\/doi.org\/(10(?:\.[0-9]{4,})?\/[^\s]*[^\s.,])$/,
  );
  if (matches) {
    return matches[1];
  }
...
const isLikeUrl = (identifier) => {
  return !!identifier.match(
    /^(https?:\/\/)?[-a-zA-Z0-9@:%._+~#=]{2,256}\.[a-z]{2,6}\b(\S*)$/i,
  );
};
...
const validateItem = (item, itemTypeFields, itemTypeCreatorTypes) => {
  ...
  if (item.creators && Array.isArray(item.creators)) {
    for (var creator of item.creators) {
      if (
        typeof itemTypeCreatorTypes.find(
          (c) => c.creatorType === creator.creatorType,
        ) === "undefined"
      ) {
...
const fetchFromPermalink = async (url) => {
  ...
  if (!response.ok) {
    throw new Error(
      `Unexpected response from the server: ${response.status}: ${response.statusText}`,
    );
  }
  return await response.json();
};
...
const getOneTimeBibliographyOrFallback = async (
  itemsCSL,
  citationStyleXml,
  ...
  if (styleHasBibliography) {
    ...
    bibliographyItems = itemsCSL.map((item) => ({
      id: item.id,
      value: render.allClusters[item.id],
    }));
  }
...
const isSentenceCase = (val) => {
  ...
  if (matches && matches.length > 2) {
    ...
    if (remainingWords) {
      remainingWords
        .filter(
          (word) => word.length >= 4 || !window.CSL.SKIP_WORDS.includes(word),
        )
        .forEach((word) => {
          totalWords++;
  ...
  if (matches) {
    return matches
      .map((s) => {
        ...
        return (
          pre + sentence[0].toUpperCase() + sentence.slice(1).toLowerCase()
        );
      })
      .join(" ");
  } else {
...
const processSentenceCaseAPAItems = (items, baseMappings) => {
  const whitelist = [
    ...Object.keys(baseMappings).reduce((agg, itemType) => {
      "title" in baseMappings[itemType] &&
        agg.push(baseMappings[itemType]["title"]);
      return agg;
    }, []),
    "title",
    "shortTitle",
      ...
      value:
        typeof value === "string"
          ...
          : {
              ...value,
              itemType: (
                itemTypes.find((it) => it.itemType == value.itemType) || {}
              ).localized,
            },
...
const fetchSchema = async (apiAuthorityPart) =>
  (await fetch(`https://${apiAuthorityPart}/schema`)).json();
...
const getMetaFromSchema = (schema, locale) => {
  const itemTypes = schema.itemTypes
    ...
    .map(({ itemType }) => ({
      itemType,
      localized: schema.locales?.[locale]?.itemTypes?.[itemType] ?? itemType,
    }));
  const itemTypeFields = schema.itemTypes.reduce(
    (acc, { itemType, fields }) => {
      acc[itemType] = fields.map(({ field }) => ({
        field,
        localized: schema.locales?.[locale]?.fields?.[field] ?? field,
      }));
      return acc;
    },
    {},
  );
  const itemTypeCreatorTypes = schema.itemTypes.reduce(
    (acc, { itemType, creatorTypes }) => {
      acc[itemType] = creatorTypes.map(({ creatorType, primary }) => ({
        creatorType,
        localized:
          schema.locales?.[locale]?.creatorTypes?.[creatorType] ?? creatorType,
        primary,
      }));
      return acc;
    },
